class SessionsController < ApplicationController
  ...
  def create
  	...
  	if user && user.authenticate(params[:password]) # check if that user exists and that it can be authenticated with the password typed in the password input field
  		session[:user_id] = user.id #RAILS MAGIC: create a key-value pair in the session hash. ':user_id' is the key and the user's id is the value
  		redirect_to products_url, :notice => "Welcome, #{user.nick}"
  	else
